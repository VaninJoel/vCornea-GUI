from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLineEdit, QCheckBox, QComboBox, QLabel
import math
...
class ParameterItemWidget(QWidget):
    ...
    def __init__(self, param, value, description, default_dict):
        ...
        normalized_value = self._normalize_initial_value(value)
        ...
        self.value_widget = self.create_value_widget(param, normalized_value, description)
        ...
        current_ref = self.find_matching_reference(normalized_value)
    ...
    def _normalize_initial_value(self, value):
        """
        If the incoming value is a reference label present in defaults,
        replace it with the corresponding numeric default. Otherwise keep it.
        """
        if isinstance(value, str) and value in self.defaults:
    ...
    def get_reference_label(self):
        return self.ref_dropdown.currentText()
